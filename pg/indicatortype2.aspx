<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="indicatortype2.aspx.cs" Inherits="LMWebApp.pg.indicatortype2" %>
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
    <h1 class="page-title txt-color-blueDark" style="margin-bottom:12px;">
        <i class="fa-fw fa fa-home"></i>
        <span id="pageUrlModuleName" runat="server">> ХАРЬЯА ГАЗРУУДЫН ҮНЭЛГЭЭ</span>
    </h1>
</div>
<div class="row">
    <div class="col-md-3" style="margin-bottom:5px;">
        <a id="pTab1Add" runat="server" href="javascript:void(0);" class="btn btn-primary btn-xs" data-target="#pModal" data-toggle="modal" onclick="showAddEdit(this,'нэмэх')"><i class="fa fa-plus"></i> Үнэлгээний төрөл нэмэх</a>
    </div>
    <div class="col-md-9">
...
<script type="text/javascript">
    ...
    function showDelete(el) {
        ...
        //    data: '{"qry":"SELECT COUNT(1) FROM TBL_INDICATOR2 WHERE INDICATORTYPE_ID=' + $(el).closest('.indicatorlist').attr('data-id') + '"}',
                    ...
                    $.SmartMessageBox({
                        ...
                        if (ButtonPressed === "Тийм") {
                            globalAjaxVar = $.ajax({
                                ...
                                data: '{"qry":"DELETE FROM TBL_INDICATORTYPE2 WHERE ID=' + $(el).closest('.indicatorlist').attr('data-id') + '"}',
                                contentType: "application/json; charset=utf-8",
                                dataType: "json",
                                success: function () {
    ...
    function dataBindDataTable() {
        ...
        globalAjaxVar = $.ajax({
            type: "POST",
            url: "../ws/ServiceMain.svc/Indicatortype2Datatable",
            data: '{}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
                ...
                $($.parseJSON(data.d)).each(function (index, value) {
                    ...
                    valData += "</div>";
                    valData += "<a href=\"../#pg/indicator2.aspx?id=" + value.ID + "\" style=\"color:#FFF;\" class=\"font-md\">" + replaceDisplayToDatabase(value.NAME) + "</a>";
                    valData += "</div>";
                    valData += "</div>";
                });
        ...
        onSuccess: function (e, data) {
            ...
            if ($('#pModalHeaderLabel').text() == 'нэмэх') {
                globalAjaxVar = $.ajax({
                    type: "POST",
                    url: "../ws/ServiceMain.svc/OracleExecuteNonQuery",
                    data: '{"qry":"INSERT INTO TBL_INDICATORTYPE2 (ID, NAME, DESCRIPTION, ISACTIVE, CREATED_STAFFID, CREATED_DATE) VALUES (TBLLASTID(\'TBL_INDICATORTYPE2\'), \'' + replaceDisplayToDatabase($.trim($('#pModalInputName').val())) + '\', \'' + replaceDisplayToDatabase($.trim($('#pModalInputDescription').val())) + '\', ' + $('input[name="pModalInputIsactive"]:checked').val() + ', ' + $('#indexUserId').text() + ', sysdate)"}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function () {
                        dataBindDataTable();
                ...
                globalAjaxVar = $.ajax({
                    type: "POST",
                    url: "../ws/ServiceMain.svc/OracleExecuteNonQuery",
                    data: '{"qry":"UPDATE TBL_INDICATORTYPE2 SET NAME=\'' + replaceDisplayToDatabase($.trim($('#pModalInputName').val())) + '\', DESCRIPTION=\'' + replaceDisplayToDatabase($.trim($('#pModalInputDescription').val())) + '\', ISACTIVE=' + $('input[name="pModalInputIsactive"]:checked').val() + ', UPDATED_STAFFID=' + $('#indexUserId').text() + ', UPDATED_DATE=sysdate WHERE ID=' + $("#pModalID").text() + '"}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function () {
                        dataBindDataTable();
