<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="tushaalsaidTab3ImplModal.aspx.cs" Inherits="LMWebApp.pg.tushaalsaidTab3ImplModal" %>
<div class="modal-header">
    <button class="close" aria-hidden="true" data-dismiss="modal" type="button"> ×</button>
    <h4 class="modal-title" >Хяналтанд авсан заалтын төлөвлөгөөний биелэлт&nbsp;<span id="pTab3ImplModalHeaderLabel" runat="server"></span></h4>
</div>
<div class="modal-body">
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="form-group" style="margin-bottom:2px;">
                <span id="pTab3ModalSpanTushaal"></span>
                <br />
...
<script type="text/javascript">
    $('#pTab3Datatable > tbody  > tr').each(function () {
        if ($(this).attr('data-id') == $('#pTab3ImplModalID').text()) {
            $('#pTab3ModalSpanTushaal').html($(this).find('td:eq(1)').html());
            $('#pTab3ModalSpanHariutssanGazar').html($(this).find('td:eq(4)').html());
            ...
            $.SmartMessageBox({
                ...
                if (ButtonPressed === "Тийм") {
                    globalAjaxVar = $.ajax({
                        ...
                        data: '{"filename":"~/files/tushaal/' + $('#pTab3ImplModalAttachBtn').html() + '"}',
    ...
    $("#pTab3ImplModalImplBtnSave").click(function () {
        ...
        if ($.trim($('#pTab3ImplModalImpl').val()) == "") {
            ...
            if (replaceDisplayToDatabase($.trim($('#pTab3ImplModalImpl').val())).length > 2000) {
                ...
                if ($('#pTab3ImplModalAttachBtn').html() != 'Файл хавсаргаагүй байна...') {
                    filename = '\'СайдТушаалЗаалт<%=Request.QueryString["id"]%>-<%=Request.QueryString["yr"]%>=<%=Request.QueryString["qrtr"]%>.' + $('#pTab3ImplModalAttachInput').val().split('.').pop() + '\'';
                    filenameF = 'СайдТушаалЗаалт<%=Request.QueryString["id"]%>-<%=Request.QueryString["yr"]%>=<%=Request.QueryString["qrtr"]%>.' + $('#pTab3ImplModalAttachInput').val().split('.').pop() + '';
                }

                if ($('#pTab3ImplModalHeaderLabel').html() == 'оруулах') {
                    valQryInsUpt = 'declare vClobVal varchar2(32767) := \'' + data + '\'; begin INSERT INTO TBL_TUSHAAL_ZAALT_IMPL (ID, TUSHAAL_ZAALT_ID, YR, QRTR, FILENAME, IMPL, IMPL_CREATED_STAFFID, IMPL_CREATED_DATE) VALUES (TBLLASTID(\'TBL_TUSHAAL_ZAALT_IMPL\'), <%=Request.QueryString["id"]%>, <%=Request.QueryString["yr"]%>, <%=Request.QueryString["qrtr"]%>, ' + filename + ', vClobVal, ' + $.trim($('#indexUserId').html()) + ', SYSDATE); end;';
                }
                else if ($('#pTab3ImplModalHeaderLabel').html() == 'засах') {
                    valQryInsUpt = 'declare vClobVal varchar2(32767) := \'' + data + '\'; begin UPDATE TBL_TUSHAAL_ZAALT_IMPL SET IMPL=vClobVal, FILENAME=' + filename + ', IMPL_MODIFY_STAFFID=' + $.trim($('#indexUserId').html()) + ', IMPL_MODIFY_DATE=SYSDATE WHERE TUSHAAL_ZAALT_ID=<%=Request.QueryString["id"]%> AND YR=<%=Request.QueryString["yr"]%> AND QRTR=<%=Request.QueryString["qrtr"]%>; end;';
                }
                globalAjaxVar = $.ajax({
                    type: "POST",
                    ...
                    success: function () {
                        if ($('#pTab3ImplModalAttachInput').val() != '') {
                            ...
                            var choice = {};
                            choice.url = "pg/UploadFile.ashx?folder=tushaal&filename=" + filenameF;
                            choice.type = "POST";
                        ...
                        $('#pTab3Datatable > tbody  > tr').each(function () {
                            if ($(this).attr('data-id') == $('#pTab3ImplModalID').text()) {
                                $('td:eq(5)', this).find('a').html('<i class="glyphicon glyphicon-check txt-color-green font-xl"></i>');
                                if (filename != 'null') {
                                    $('td:eq(6)', this).html('<a href="../files/tushaal/' + filenameF + '" class="btn btn-link btn-xs" style="padding:0px; border:none;" download="" title="Хавсаргасан файл татах"><i class="fa fa-paperclip fa-lg"></i></a>');
                                }
                                return false;
                            }
                        });
        ...
        $.SmartMessageBox({
            ...
            if (ButtonPressed === "Тийм") {
                globalAjaxVar = $.ajax({
                    type: "POST",
                    url: "../ws/ServiceMain.svc/OracleExecuteNonQuery",
                    data: '{"qry":"DELETE FROM TBL_TUSHAAL_ZAALT_IMPL WHERE TUSHAAL_ZAALT_ID=<%=Request.QueryString["id"]%> AND YR=<%=Request.QueryString["yr"]%> AND QRTR=<%=Request.QueryString["qrtr"]%>"}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function () {
                        ...
                        $('#pTab3Datatable > tbody  > tr').each(function () {
                            if ($(this).attr('data-id') == $('#pTab3ImplModalID').text()) {
                                $('td:eq(5)', this).find('a').html('<i class="glyphicon glyphicon-edit txt-color-red font-xl"></i>');
                                $('td:eq(6)', this).html('');
                                return false;
                            }
                        });
